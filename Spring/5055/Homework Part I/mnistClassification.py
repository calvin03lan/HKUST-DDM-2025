...
class Softmax(Node):
    ...
    def backward(self, delta):
        return self.y * (delta - (delta * self.y).sum(axis=1, keepdims=True))
...
class CrossEntropy(Node):
    ...
    def forward(self, x, l):
        self.x = x
        self.l = l
        return -(l * np.log(x + 1e-9)).sum(axis=1)

    def backward(self, delta):
        return delta.reshape(-1, 1) * -self.l / (self.x + 1e-9)
...
def load_MNIST():
    ...
    for obj in objects:
        if not os.path.isfile(path + obj):
            urllib.request.urlretrieve(base + obj + end, path + obj + end)
...
if __name__ == '__main__':
    ...
    net = [Linear(dim_img, 128), Sigmoid(), Linear(128, num_digit), Softmax(), CrossEntropy(), Mean()]
